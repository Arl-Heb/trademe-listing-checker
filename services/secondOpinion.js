const AiClient = require('@anthropic-ai/sdk');

// lets the model be swapped without a code change if that's ever needed
...
// forcing the reply into this shape means we don't have to parse free text out
// of the response, and it keeps the model from padding out its answer
...
function getClient() {
  ...
  return new AiClient({ apiKey: process.env.AI_API_KEY });
}

async function getSecondOpinion(listing) {
  ...
  try {
    ...
    // an outage, a bad key, or a timeout shouldn't take the whole check down,
    // the pattern rules already ran and can stand on their own
    console.error('Second opinion request failed:', err.message);
    return null;
  }
}

module.exports = { getSecondOpinion, isConfigured };
